...
export function QuizSelectScreen({ onSelect, onViewCollection }: { onSelect: (id: string) => void, onViewCollection: () => void }) {
  ...
  const hideGlitchTimeoutRef = React.useRef<ReturnType<typeof setTimeout> | null>(null);
  ...
  React.useEffect(() => {
    if (unlockedCount < 10) {
      ...
      if (hideGlitchTimeoutRef.current) {
        clearTimeout(hideGlitchTimeoutRef.current);
        hideGlitchTimeoutRef.current = null;
      }
      return;
    }

    // Show True Reality briefly every 2-3 minutes.
    const intervalRandom = () => Math.random() * 60000 + 120000; // 2 to 3 minutes
    const visibleDurationRandom = () => Math.random() * 4000 + 6000; // 6 to 10 seconds
    ...
    const triggerGlitch = () => {
      ...
      hideGlitchTimeoutRef.current = setTimeout(() => setGlitchVisible(false), visibleDurationRandom());
      timerId = setTimeout(triggerGlitch, intervalRandom());
    };
